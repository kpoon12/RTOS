...
void spi2_LowLevel_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(spi2_SPI_SCK_GPIO_CLK | spi2_SPI_MISO_GPIO_CLK |
                         spi2_SPI_MOSI_GPIO_CLK | adc1_CS_GPIO_CLK | dac1_CS_GPIO_CLK, ENABLE);
  ...
  GPIO_InitStructure.GPIO_Pin = dac1_CS_PIN |ldac1_PIN | clr1_PIN;
  ...
  GPIO_InitStructure.GPIO_Pin =  dac2_CS_PIN | ldac2_PIN | clr2_PIN | adc1_CS_PIN | adc2_CS_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
...
void spi2_LowLevel_DeInit(void)
{
  ...
  GPIO_Init(ldac1_GPIO_PORT, &GPIO_InitStructure);
}
...
void SelectSpiDevice(uint8_t uDevice)
{
	...
	GPIO_SetBits(dac1_CS_GPIO_PORT, dac1_CS_PIN);
	GPIO_SetBits(dac2_CS_GPIO_PORT, dac2_CS_PIN);
	...
	switch(uDevice)
	{
		...
		case ANALOG_OUTPUT_DAC:
			GPIO_ResetBits(dac2_CS_GPIO_PORT, dac2_CS_PIN);
			break;
		case CURRENT_SERVO_DAC:
			GPIO_ResetBits(dac1_CS_GPIO_PORT, dac1_CS_PIN);
			break;
		default:
			// INVALID SPI DEVICE
			break;
